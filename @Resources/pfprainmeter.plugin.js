...
module.exports = (_ => {
	...
	return !global.ZeresPluginLibrary ? class {
		...
		const plugin = (Plugin, Library) => {
			...
			return class pfprainmeter extends Plugin {
				constructor() {
					super();
				}

				start() {
					this.interv = setInterval(this.doStuff, 20000);
					this.doStuff();
				}
				...
				doStuff() {
					//if (this.settings.path instanceof File) {
						...
						require("fs").writeFile(require('os').homedir() + "\\Documents\\Rainmeter\\Skins\\Lovelace\\@Resources\\Images\\DiscordAv.txt", src.replace(/(?:\?size=\d{3,4})?$/, "?size=4096"), err => {});
					//} else {
					// 	setTimeout(() => {
					// 		Toasts.show("Please set the file path in pfprainmeter's settings", { type: "info", timeout: 6000 }),
					// 		console.log(this.settings.path)
					// 	}, 3000);
					// }
				}
